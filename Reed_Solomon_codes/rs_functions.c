...
void encode_rs(unsigned char data[], int alpha_to[], int index_of[],int gg[], unsigned char ecc[])
/* take the string of symbols in data[i], i=0..(k-1) and encode systematically
 to produce 2*tt parity symbols in ecc[0]..ecc[2*tt-1]
 data[] is input and ecc[] is output in polynomial form.
 ...
 Codeword is   c(X) = data(X)*X**(nn-kk)+ b(X)          */
{
    ...
    for (i=0; i<nn-kk; i++)   ecc[i] = 0 ;
    for (i=kk-1; i>=0; i--)
    {  feedback = index_of[data[i]^ecc[nn-kk-1]] ;
        if (feedback != -1)
        { for (j=nn-kk-1; j>0; j--)
            if (gg[j] != -1)
                ecc[j] = ecc[j-1]^alpha_to[(gg[j]+feedback)%nn] ;
            else
                ecc[j] = ecc[j-1] ;
            ecc[0] = alpha_to[(gg[0]+feedback)%nn] ;
        }
        else
        { for (j=nn-kk-1; j>0; j--)
            ecc[j] = ecc[j-1] ;
    ...
    if (syn_error)       /* if errors, try and correct */
    {
        ...
        if (l[u]<=tt)         /* can correct error */
        {
            ...
            if (count==l[u])    /* no. roots = degree of elp hence <= tt errors */
            {
                ...
                for (i=0; i<nn; i++)
                { err[i] = 0 ;
                    if (recd[i]!=0xff)        /* convert recd[] to polynomial form */
...
void GenerateRS128(const unsigned char* pPayload, int payloadLen, unsigned char* pResult, int* pResultLen )
{
    ...
    encode_rs((unsigned char*)pPayload, alpha_to, index_of, gg, Ecc);
    
    *pResultLen = nn-kk;
    
    memcpy(pResult, pPayload, payloadLen);
    memcpy(&pResult[payloadLen], Ecc, *pResultLen);
}




int ValidateRS128(unsigned char* pInput, int InputLen, unsigned char* pPayLoad, int PayLoadLen)
{
    int alpha_to [nn+1], index_of [nn+1], gg [nn-kk+1], i, ret;
    ...
    memcpy(recd, &pInput[kk], nn-kk);
    memcpy(&recd[nn-kk], pInput, kk);
    
    /* put recd[i] into index form */
    for (i=0; i<nn; i++)
        recd[i] = index_of[recd[i]] ;
    
    /* perform decoding/detection, recd[] is returned in polynomial form */
    ret = decode_rs(recd, alpha_to, index_of) ;
    
    memcpy(pPayLoad, &recd[nn-kk], kk);
    
    if (ret != 0)
        return FAILED_CORRECTING;
    
    for (i=0; i<kk; i++) pPayLoad[i] = recd[i+nn-kk];
