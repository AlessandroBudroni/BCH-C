                ...
                unsigned int len, uint8_t *ecc)
{
    ...
    unsigned long m;
    //uint32_t w, r[l+1];
    size_t sizeofr = (l + 1) * sizeof(uint32_t);
    uint32_t w, *r = malloc(sizeofr);
    ...
    if (ecc) {
        ...
        memset(bch->ecc_buf, 0, sizeofr);
    }
    
    /* process first unaligned data bytes */
    m = ((unsigned long) ((size_t) data)) & 3;
    if (m) {
        mlen = (len < (4-m)) ? len :4-(int)m;
                               ...
                               unsigned int *sol, int nsol)
{
    ...
    int rem, c, r, p, k, *param = malloc(sizeof(int) * m);
...
static int build_deg2_base(struct bch_control *bch)
{
    ...
    size_t sizeofxi = sizeof(unsigned int) * m;
    unsigned int sum, x, y, remaining, ak = 0, *xi = malloc(sizeofxi);
...
/**
 * ValidateBCH128 - validate BCH code for a 128-bit message. The parameters of this funciton are designed for a 128-bit message. This function MUST NOT be used for other messges-length.
